...
- **📡 BLE Mesh Sync** — Multiple responder phones discover each other via Bluetooth Low Energy or Wifi-direct and automatically share triage cards. No pairing dialogs, no server.
...
## How We Use Gemma 4 (Verified, Not Theoretical)

**Model:** Gemma 4 E4B (4.5B effective parameters, 8B with embeddings, 9.6GB via Ollama)

**1. Native Audio Encoder (300M params)**
Gemma 4 E4B has a built-in audio encoder that processes speech directly — no separate ASR model needed. We record 16kHz mono PCM audio, wrap it in a WAV header, base64 encode it, and send it via Ollama's multimodal API. The model transcribes speech and detects language in a single inference call. Each second of audio = 25 tokens. Max clip: 30 seconds.
...
For triage card generation, we define a `generate_triage_card` tool with a JSON schema (urgency_level, people_count, needs_categories, detected_language, assessment_summary). Gemma 4 responds with structured `tool_calls` containing the arguments — no regex parsing, no brittle JSON extraction. Confirmed working: model returns `tool_calls: 1` with correct structured data.

**4. 140+ Language Support**
Gemma 4's multilingual training enables transcription and translation across languages without separate models per language pair. One 9.6GB model handles everything.

**5. 128K Context Window**
The full multi-turn encounter (speech turns, photo assessments, notes) fits in a single context, so the triage card captures the complete picture.

**6. System Role Support**
Gemma 4 natively supports the `system` role for structured agent behavior, used in our triage agent prompts.
...
├──────────┬──────────┬───────────┬───────────────────┤
│ Audio    │ Vision   │ Translate │ Triage Agent      │
│ Recorder │ Analyzer │ Engine    │ (Function Calling)│
│ (16kHz   │ (base64  │ (detect + │ (Ollama tools API)│
│  WAV)    │  JPEG)   │  translate)│                  │
├──────────┴──────────┴───────────┴───────────────────┤
│         OllamaClient (NDJSON, tools support)         │
├─────────────────────────────────────────────────────┤
│         Gemma 4 E4B via Ollama (localhost:11434)     │
├─────────────────────────────────────────────────────┤
│  Room DB  │  BLE Mesh Sync  │  Android TTS (offline)│
└───────────┴─────────────────┴───────────────────────┘
```
...
**Triage Pipeline:** Full conversation context → Gemma 4 with `tools` parameter → model returns `tool_calls[0].function.arguments` → structured TriageCard → Room DB → BLE mesh broadcast.
...
- **Triage card generation**: 7 seconds, correct urgency/needs/summary via function calling ✓
- **Vision analysis**: Structural damage photos assessed with severity ratings ✓
